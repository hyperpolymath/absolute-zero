...
|===
|Line |Identifier |Disposition |Justification
|56 |`+mkdir+` |§(c) AXIOM |Opaque POSIX primitive — no executable body
in the model.
...
These claim provable existence / equality facts that should follow from
the §(c) primitives once the model is concretely defined. They need a
discharge PR — see §(d) DEBT below.
